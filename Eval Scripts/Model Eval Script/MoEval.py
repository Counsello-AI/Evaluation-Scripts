        ...
        for q_num, q_data in tqdm(questions.items(), desc="Generating predictions"):
            ...
            try:
                ...
                if "out of memory" in str(e):
                    ...
                    # Move model back to GPU for next question
                    model.to(device)
                else:
                    raise e
...
def main():
    ...
    try:
        # Setting memory-saving configurations (added this through an error)
        torch.backends.cudnn.benchmark = False
        torch.backends.cudnn.deterministic = True
        
        # Setting environment variable for memory allocation
